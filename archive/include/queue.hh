...
struct queue {
  ...
  bool enqueue(block *ptr, block *loc = nullptr) {
    ...
    auto prev = this->head.exchange(nullptr, std::memory_order_acquire);
    if (prev == nullptr) {
      return false;
    }
    loc->next = prev;
    auto check = this->head.exchange(ptr, std::memory_order_release);
  ...
  block *dequeue(void) {
    ...
    auto check = this->head.exchange(next, std::memory_order_release);
    CmiAssertMsg(check == nullptr, "bad atomic exchange result");
